...
public class SharedInventoryMod implements ModInitializer {
    ...
    /** 翻页网络包通道 */
    public static final Identifier PAGE_UPDATE_ID = Identifier.of(MOD_ID, "page_update");
    /** 标签更新网络包通道 */
    public static final Identifier LABEL_UPDATE_ID = Identifier.of(MOD_ID, "label_update");
    /** 打开背包网络包通道 (穿戴后按键触发) */
    public static final Identifier OPEN_BACKPACK_ID = Identifier.of(MOD_ID, "open_backpack");
    /** 工具切换网络包通道 */
    public static final Identifier TOOL_UPDATE_ID = Identifier.of(MOD_ID, "tool_update");

    public static final RegistryKey<ItemGroup> SHARED_INVENTORY_GROUP_KEY = RegistryKey.of(RegistryKeys.ITEM_GROUP, Identifier.of(MOD_ID, "shared_inventory_mod"));

    public static final ItemGroup SHARED_INVENTORY_GROUP = Registry.register(
            Registries.ITEM_GROUP,
            SHARED_INVENTORY_GROUP_KEY,
            FabricItemGroup.builder()
                    .icon(() -> new ItemStack(ModObjects.SHARED_INVENTORY_BACKPACK))
                    .displayName(net.minecraft.text.Text.translatable("itemGroup.shared_inventory_mod.shared_inventory_mod"))
    ...
    public void onInitialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(PageUpdatePayload.ID, (payload, context) -> {
            context.server().execute(() -> PageUpdatePayload.handle(payload, context.player()));
        });

        ServerPlayNetworking.registerGlobalReceiver(LabelUpdatePayload.ID, (payload, context) -> {
            context.server().execute(() -> LabelUpdatePayload.handle(payload, context.player()));
        });

        ServerPlayNetworking.registerGlobalReceiver(ToolUpdatePayload.ID, (payload, context) -> {
            context.server().execute(() -> ToolUpdatePayload.handle(payload, context.player()));
        });
    }
}
