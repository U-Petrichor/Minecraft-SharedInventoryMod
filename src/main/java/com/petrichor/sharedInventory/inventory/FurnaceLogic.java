...
public class FurnaceLogic {
    ...
    public void tick(World world) {
        ...
        if (world != null && !world.isClient) {
            ...
            if (isBurning() || (!fuelStack.isEmpty() && !inputStack.isEmpty())) {
                // 1.21 API: 使用 SingleStackRecipeInput 代替 Inventory
                SingleStackRecipeInput recipeInput = new SingleStackRecipeInput(inputStack);
                Optional<RecipeEntry<SmeltingRecipe>> recipeEntry = world.getRecipeManager().getFirstMatch(RecipeType.SMELTING, recipeInput, world);

                if (!isBurning() && canAcceptRecipeOutput(recipeEntry.map(RecipeEntry::value).orElse(null), recipeInput, world)) {
                ...
                if (isBurning() && canAcceptRecipeOutput(recipeEntry.map(RecipeEntry::value).orElse(null), recipeInput, world)) {
                    if (this.totalCookTime == 0) {
                        this.totalCookTime = getCookTime(recipeEntry);
                    }
                    ++this.cookTime;
                    if (this.cookTime == this.totalCookTime) {
                        this.cookTime = 0;
                        this.totalCookTime = getCookTime(recipeEntry);
                        if (craftRecipe(recipeEntry.map(RecipeEntry::value).orElse(null), recipeInput, world)) {
                            isDirty = true;
                        }
                    }
                } else {
    ...
    private int getCookTime(Optional<RecipeEntry<SmeltingRecipe>> recipeEntry) {
        return recipeEntry.map(entry -> entry.value().getCookingTime()).orElse(DEFAULT_COOK_TIME);
    }
