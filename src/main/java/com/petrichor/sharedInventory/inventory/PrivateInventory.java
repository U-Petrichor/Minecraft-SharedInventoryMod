...
public class PrivateInventory implements Inventory {
    ...
    public void readLabelsFromNbt(NbtCompound nbt) {
        pageLabels.clear();
        if (nbt.contains("PageLabels")) {
            NbtCompound labelsNbt = nbt.getCompound("PageLabels").orElse(new NbtCompound());
            for (String key : labelsNbt.getKeys()) {
                try { int page = Integer.parseInt(key); if (page >= 1 && page <= MAX_PAGE) { pageLabels.put(page, labelsNbt.getString(key).orElse("")); } } catch (NumberFormatException ignored) {}
            }
        }
    }
    ...
    public NbtList toNbtList(RegistryWrapper.WrapperLookup registryLookup) {
        ...
        for (int slot = 0; slot < STACK_SIZE; slot++) {
            ...
            if (!stack.isEmpty()) {
                ...
                ItemStack.CODEC.encodeStart(registryLookup.getOps(net.minecraft.nbt.NbtOps.INSTANCE), stack)
                        .result().ifPresent(encoded -> itemTag.copyFrom((NbtCompound) encoded));
                nbtList.add(itemTag);
            }
        }
        return nbtList;
