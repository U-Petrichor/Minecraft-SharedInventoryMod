...
public class SharedInventoryBackpack extends Item implements NamedScreenHandlerFactory {
    ...
    public @Nullable ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        ...
        BackpackInventory inventory = createLinkedInventory(backpackStack, server);
        if (inventory == null) {
            player.sendMessage(net.minecraft.text.Text.translatable("message.shared_inventory_mod.shared_inventory_backpack.message1"), true);
            return null;
        }
    ...
    public Text getDisplayName() {
        return net.minecraft.text.Text.translatable("item.shared_inventory_mod.shared_inventory_backpack");
    }
    ...
    public void linkToChest(ItemStack stack, SharedInventoryChestBlockEntity blockEntity) {
        if (blockEntity != null && blockEntity.getWorld() != null) {
            ...
            NbtComponent.set(DataComponentTypes.CUSTOM_DATA, stack, nbt -> {
                nbt.putIntArray("linkedCoreId", Uuids.toIntArray(coreId));
    ...
    public static BackpackInventory createLinkedInventory(ItemStack stack, MinecraftServer server) {
        ...
        int[] storedId = nbt.getIntArray("linkedCoreId").orElse(null);
        if (storedId != null && storedId.length == 4) {
            UUID coreId = Uuids.toUuid(storedId);
        ...
        NbtComponent.set(DataComponentTypes.CUSTOM_DATA, stack,
                data -> data.putIntArray("linkedCoreId", Uuids.toIntArray(coreId)));
    ...
    public static SharedInventoryChestBlockEntity readLinkedBlockEntity(ItemStack stack, MinecraftServer server) {
        ...
        BlockPos pos = BlockPos.fromLong(nbt.getLong("linkedBlockEntityPos").orElse(0L));

        // 优先按维度精确查找 (新格式)
        if (nbt.contains("linkedDimension")) {
            try {
                Identifier dimId = Identifier.of(nbt.getString("linkedDimension").orElse(""));
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(net.minecraft.text.Text.translatable("item.shared_inventory_mod.shared_inventory_backpack.tooltip"));
    }
}
