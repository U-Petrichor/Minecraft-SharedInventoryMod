...
public record OpenBackpackPayload() implements CustomPayload {
    ...
    public static void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ID, (payload, context) -> {
            context.server().execute(() -> handleOpenBackpack(context.player(), context.server()));
        });
    }

    private static void handleOpenBackpack(ServerPlayerEntity player, MinecraftServer server) {
        ...
        if (backpackStack == null || !(backpackStack.getItem() instanceof SharedInventoryBackpack)) {
            player.sendMessage(net.minecraft.text.Text.translatable("message.shared_inventory_mod.backpack.not_equipped"), true);
            return;
        }

        BackpackInventory inventory = SharedInventoryBackpack.createLinkedInventory(
                backpackStack, server);
