...
public class SharedInventoryScreen extends HandledScreen<SharedInventoryScreenHandler> implements SharedInventoryScreenHandler.ClientCallback {
    ...
    public void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        if (activeTool == ToolType.FURNACE) {
            ...
            context.drawTexture(FURNACE_TEXTURE, furnaceBaseX + 20, furnaceBaseY + 19, 79, 34, 24, 16, 256, 256);
            // Arrow — progress fill
            int n = this.handler.getCookProgress();
            if (n > 0) {
                context.drawTexture(FURNACE_TEXTURE, furnaceBaseX + 20, furnaceBaseY + 19, 176, 14, n + 1, 16, 256, 256);
            }
        }
        ...
        if (activeTool == ToolType.BREWING) {
            ...
            if (fuelWidth > 0) {
                context.drawTexture(BREWING_TEXTURE, brewingBaseX + 20, brewingBaseY + 22, 176, 29, fuelWidth, 4, 256, 256);
            }
            ...
            if (brewTime > 0) {
                ...
                if (bubbleHeight > 0) {
                    context.drawTexture(BREWING_TEXTURE, brewingBaseX + 23, brewingBaseY + 20 - bubbleHeight, 185, 29 - bubbleHeight, 12, bubbleHeight, 256, 256);
                }

                // 酿造箭头
                int arrowProgress = (400 - brewTime) * 28 / 400;
                if (arrowProgress > 0) {
                    context.drawTexture(BREWING_TEXTURE, brewingBaseX + 60, brewingBaseY + 2, 176, 0, 9, arrowProgress, 256, 256);
                }
            }
        }
