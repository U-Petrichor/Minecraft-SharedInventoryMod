...
public class SharedInventoryScreen extends HandledScreen<SharedInventoryScreenHandler> implements SharedInventoryScreenHandler.ClientCallback {
    ...
    public void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, i, j, 0, 0, this.backgroundWidth, this.backgroundHeight, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        ...
        context.drawTexture(RenderLayer::getGuiTextured, overlayTexture, overlayX, overlayY, 0, 0, OVERLAY_REGION_W, OVERLAY_REGION_H, OVERLAY_REGION_W, OVERLAY_REGION_H);
        ...
        if (activeTool == ToolType.FURNACE) {
            ...
            if (this.handler.isBurning()) {
                ...
                context.drawGuiTexture(RenderLayer::getGuiTextured, FURNACE_LIT_PROGRESS_TEXTURE, 14, 14, 0, 14 - flameHeight, furnaceBaseX + 3, furnaceBaseY + 35 - flameHeight, 14, flameHeight);
            }

            int cookProgress = this.handler.getCookProgress();
            if (cookProgress > 0) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, FURNACE_BURN_PROGRESS_TEXTURE, 24, 16, 0, 0, furnaceBaseX + 20, furnaceBaseY + 19, cookProgress + 1, 16);
            }
        }
        ...
        if (activeTool == ToolType.BREWING) {
            ...
            if (fuelWidth > 0) {
                // 1.21.2 API: drawGuiTexture(renderLayer, texture, texWidth, texHeight, u, v, x, y, width, height)
                context.drawGuiTexture(RenderLayer::getGuiTextured, BREWING_FUEL_LENGTH_TEXTURE, 18, 4, 0, 0, brewingBaseX + 20, brewingBaseY + 22, fuelWidth, 4);
            }
            ...
            if (brewTime > 0) {
                ...
                if (bubbleHeight > 0) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, BREWING_BUBBLES_TEXTURE, 12, 29, 0, 29 - bubbleHeight, brewingBaseX + 23, brewingBaseY + 20 - bubbleHeight, 12, bubbleHeight);
                }

                int arrowProgress = (400 - brewTime) * 28 / 400;
                if (arrowProgress > 0) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, BREWING_BREW_PROGRESS_TEXTURE, 9, 28, 0, 0, brewingBaseX + 60, brewingBaseY + 2, 9, arrowProgress);
                }
            }
        }
