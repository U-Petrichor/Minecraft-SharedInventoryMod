...
public class SharedInventoryScreen extends HandledScreen<SharedInventoryScreenHandler> {
    ...
    protected void init() {
        super.init();
        shardInventoryPlayerEntity.shared_inventory1_18_2$setSearchIndex(0);
        ...
        this.searchField = new TextFieldWidget(
                this.textRenderer,
                ((this.width - this.backgroundWidth) / 2) + 8,
                ((this.height - this.backgroundHeight) / 2) + 162,
                100,
                12,
                Text.of("")
        ...
        addDrawableChild(new ButtonWidget(
                ((this.width - this.backgroundWidth) / 2)+110, // X 坐标
                ((this.height - this.backgroundHeight) / 2)+162, // Y 坐标
                34, // 宽度
                12, // 高度
                new TranslatableText("Search"),
    ...
    private void performSearch() {
        ...
        SharedInventoryPlayerEntity entity = (SharedInventoryPlayerEntity) client.player;
        int currentIndex = entity.shared_inventory1_18_2$getSearchIndex();
        int found = entity.shared_inventory1_18_2$getPrivateInventory().findNextItemIndex(currentIndex, searchText);

        if (found != -1) {
            int newPage = (found / 72) + 1;
            entity.shared_inventory1_18_2$getPrivateInventory().setCurrentPage(newPage);
            entity.shared_inventory1_18_2$setSearchIndex(found + 1);

            PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
            buf.writeInt(newPage);
            buf.writeInt(found + 1);
            ClientPlayNetworking.send(SharedInventoryMod.STATE_UPDATE_ID, buf);
        } else {
            client.player.sendMessage(new TranslatableText("No matching item found"), true);
            entity.shared_inventory1_18_2$setSearchIndex(0);
            searchField.setText("");
        }
    }
}
